...
class DataLoaderTest(object):
    ...
    def __init__(self, data_path, tokenizer, args, cuda=True, batch_size=128, sentences=None,claim=None):
        self.cuda = cuda
    ...
    def read_file(self, data_path):
        inputs = list()
        ids = list()
        evi_list = self.sentences
        print(len(evi_list))
        print('read_file')
        for sentence in evi_list:
            inputs.append([self.process_sent(self.claim), self.process_sent(sentence)])
            ids.append(self.process_wiki_title(self.claim))
        # with open(data_path) as fin:
        #     for step, line in enumerate(fin):
